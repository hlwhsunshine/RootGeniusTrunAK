package android.support.p015v4.graphics;
...
public final class ColorUtils {
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        return Color.rgb(ColorUtils.constrain(round, 0, 255), ColorUtils.constrain(round2, 0, 255), ColorUtils.constrain(round3, 0, 255));
    }
    ...
    public static int LABToColor(@FloatRange(from = 0.0d, to = 100.0d) double d, @FloatRange(from = -128.0d, to = 127.0d) double d2, @FloatRange(from = -128.0d, to = 127.0d) double d3) {
        ...
        ColorUtils.LABToXYZ(d, d2, d3, tempDouble3Array);
        return ColorUtils.XYZToColor(tempDouble3Array[0], tempDouble3Array[1], tempDouble3Array[2]);
    }
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull float[] fArr) {
        ...
        fArr[0] = ColorUtils.constrain(f, 0.0f, 360.0f);
        fArr[1] = ColorUtils.constrain(f2, 0.0f, 1.0f);
        fArr[2] = ColorUtils.constrain(f5, 0.0f, 1.0f);
    }
    ...
    public static void RGBToLAB(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull double[] dArr) {
        ColorUtils.RGBToXYZ(i, i2, i3, dArr);
        ColorUtils.XYZToLAB(dArr[0], dArr[1], dArr[2], dArr);
    }
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3) {
        ...
        return Color.rgb(ColorUtils.constrain((int) Math.round((d4 > 0.0031308d ? (Math.pow(d4, 0.4166666666666667d) * 1.055d) - 0.055d : d4 * 12.92d) * 255.0d), 0, 255), ColorUtils.constrain((int) Math.round((d5 > 0.0031308d ? (1.055d * Math.pow(d5, 0.4166666666666667d)) - 0.055d : 12.92d * d5) * 255.0d), 0, 255), ColorUtils.constrain((int) Math.round((d6 > 0.0031308d ? (1.055d * Math.pow(d6, 0.4166666666666667d)) - 0.055d : 12.92d * d6) * 255.0d), 0, 255));
    }
    ...
    public static void XYZToLAB(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3, @NonNull double[] dArr) {
        ...
        double pivotXyzComponent = ColorUtils.pivotXyzComponent(d / XYZ_WHITE_REFERENCE_X);
        double pivotXyzComponent2 = ColorUtils.pivotXyzComponent(d2 / XYZ_WHITE_REFERENCE_Y);
        double pivotXyzComponent3 = ColorUtils.pivotXyzComponent(d3 / XYZ_WHITE_REFERENCE_Z);
    ...
    public static void blendHSL(@NonNull float[] fArr, @NonNull float[] fArr2, @FloatRange(from = 0.0d, to = 1.0d) float f, @NonNull float[] fArr3) {
        ...
        float f2 = 1.0f - f;
        fArr3[0] = ColorUtils.circularInterpolate(fArr[0], fArr2[0], f);
    ...
    public static int calculateMinimumAlpha(@ColorInt int i, @ColorInt int i2, float f) {
        ...
        if (Color.alpha(i2) != 255) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(i2));
        } else if (ColorUtils.calculateContrast(ColorUtils.setAlphaComponent(i, 255), i2) < ((double) f)) {
            return -1;
        } else {
            int i5 = 0;
            while (i5 <= 10 && i4 - i3 > 1) {
                int i6 = (i3 + i4) / 2;
                if (ColorUtils.calculateContrast(ColorUtils.setAlphaComponent(i, i6), i2) >= ((double) f)) {
                    i4 = i6;
                    i6 = i3;
                }
                i5++;
