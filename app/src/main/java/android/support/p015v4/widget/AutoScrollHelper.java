...
public abstract class AutoScrollHelper implements OnTouchListener {
    ...
    public AutoScrollHelper(View view) {
        ...
        setRampUpDuration(AdError.PLACEMENT_ERROR);
        setRampDownDuration(AdError.PLACEMENT_ERROR);
    }
    ...
    private float computeTargetVelocity(int i, float f, float f2, float f3) {
        ...
        f4 *= f3;
        return edgeValue > 0.0f ? AutoScrollHelper.constrain(edgeValue * f4, f5, f6) : -AutoScrollHelper.constrain((-edgeValue) * f4, f5, f6);
    }
    ...
    private float getEdgeValue(float f, float f2, float f3, float f4) {
        float f5;
        float constrain = AutoScrollHelper.constrain(f * f2, 0.0f, f3);
        ...
        return AutoScrollHelper.constrain(f5, -1.0f, (float) DEFAULT_RELATIVE_VELOCITY);
    }
